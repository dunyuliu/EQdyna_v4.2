...
fltxyz(1,3,1)=fzmin
fltxyz(2,3,1)=fzmax
fltxyz(1,4,1)=fstrike*pi/180.0d0
fltxyz(2,4,1)=fdip*pi/180.0d0
...
allocate(nsmp(2,nftmx,ntotft),fnft(nftmx,ntotft),miuonf(nftmx),vponf(nftmx),un(3,nftmx,ntotft),&
			...
			arn4m(nftmx,ntotft),slp4fri(nftmx,ntotft),fltslp(10,nftmx,ntotft),state(nftmx,ntotft))
!ALLOCATE 3DMPI ARRAYS			
allocate(fltgm(nftmx))  !MPIxyz
fltgm=0  !MPIxyz
!INITIATION before calling meshgen
nsmp = 0    !initialize here
fnft = 1000.0d0 !as failure time initialization: should larger than actual!
miuonf=0.0d0
vponf=0.0d0
fric = 0.0d0
un = 0.0d0
...
if(n4out>0) then
	do i=1,n4out
		...
		do j=1,locplt
			write(51,'( 7e18.7e4)') dout(1,j),dout((i-1)*6+2,j), &
...
!------------------------RUPTURE TIME CONTOURS----------------------!
 if(nftnd(1) > 0) then
	 ...
	 write(ioutrt1,'(14e18.7e4)') ((x(j,nsmp(1,i,1)),j=1,3),fnft(i,1),&
	         (fltslp(j,i,1),j=1,10),i=1,nftnd(1))
	 close(ioutrt1)
 endif
!time2=MPI_WTIME()
