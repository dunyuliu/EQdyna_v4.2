...
! enddo
! close(3001)
!open(3001,file='patchnumber.txt', form='formatted',status='old')
!read(3001,*) ipatch
!close(3001)
!open(3001,file='direction.txt',form='formatted',status='old')
!read(3001,*) idirection
!close(3001)
!-------------------------------------------------------------------!
write(mm,'(i6)') me
mm = trim(adjustl(mm))
...
x4nds=reshape((/-4.4392,-1.9846,0.0,&
				-7.5493,11.1297,0.0,&
				8.5461,-1.7103,0.0,&
				-0.7978,-1.6538,0.0,&
				0.8575,-4.6750,0.0,&
...
!-------------------------------TPV 27v2-------------------------------!
						fric(1,nftnd(ift),ift) = 1000.0d0
						fric(2,nftnd(ift),ift) = 1000.0d0
						! if (xcoor<2.25d3.and.xcoor>-2.25d3.and.zcoor<-3.4d3+2.25d3.and.zcoor>-3.4d3-2.25d3)then
                                                        ! tmp1 = 1.0d0
                                                        ! tmp2 = 1.0d0
                                           ! if (abs(xcoor)>=1.25d3) then
                                               ! tmp1 = (2.25d3-abs(xcoor))/1.0d3
                                                ! endif
                                                ! if (abs(zcoor--3.4d3)>=1.25d3) then
                                               ! tmp2 = (2.25d3-abs(zcoor--3.4d3))/1.0d3
                                                ! endif
							 ! fric(1,nftnd(ift),ift) = 0.4d0   !mud
                                                         ! fric(2,nftnd(ift),ift) = 0.4d0 - 0.1d0*tmp1*tmp2
						! endif		
						if (xcoor<3.0d3.and.xcoor>-3.0d3.and.zcoor>-6.4d3.and.zcoor<-0.4d3)then
							tmp1 = 1.0d0
							tmp2 = 1.0d0
							if (abs(xcoor-2.0d3)<1.0d3.and.abs(zcoor--3.4d3)<1.0d3) then
								tmp1 = 0.0d0
								tmp2 = 0.0d0 
							endif
							if (abs(xcoor--2.0d3)<1.0d3.and.abs(zcoor--3.4d3)<1.0d3) then
							...
							if (abs(xcoor-0.0d3)<1.0d3.and.abs(zcoor--1.4d3)<1.0d3) then
								tmp1 = 0.0d0
								tmp2 = 0.0d0 
							endif		
							if (abs(xcoor-0.0d3)<1.0d3.and.abs(zcoor--5.4d3)<1.0d3) then
								tmp1 = 0.0d0
								tmp2 = 0.0d0 
							endif								
											   ! tmp1 = (2.25d3-abs(xcoor))/1.0d3
						...
						if (C_elastic==1) then
							...
							! endif
							fric(7,nftnd(ift),ift)=-100.0d6
							fric(8,nftnd(ift),ift)=35.0d6
						endif
!-------------------------------Tianjin ref-------------------------------!						
						! if (C_elastic==1) then
...
!TPV8
!Feb.19.2016
!For Cushing EQ dynamic inversion				
!Oct.25.2019
                if (abs(xc(3))<=0.2d3) then
					mat(nelement,1)=3.1d3
					mat(nelement,2)=1.5d3
					mat(nelement,3)=2.308818d3
				elseif (abs(xc(3))<=0.4d3) then
					mat(nelement,1)=3.3d3!3.06d3
					mat(nelement,2)=1.679882d3
					mat(nelement,3)=2.345188d3
				elseif (abs(xc(3))<=0.6d3) then
					mat(nelement,1)=3.5d3!3.06d3
					mat(nelement,2)=1.859764d3
					mat(nelement,3)=2.379941d3	
				elseif (abs(xc(3))<=0.8d3) then
					mat(nelement,1)=3.7d3!3.06d3
					mat(nelement,2)=2.039646d3
					mat(nelement,3)=2.413235d3	
				elseif (abs(xc(3))<=1.0d3) then
					mat(nelement,1)=4.0d3!3.06d3
					mat(nelement,2)=2.309469d3
					mat(nelement,3)=2.460832d3
				elseif (abs(xc(3))<=1.2d3) then
					mat(nelement,1)=4.344d3!3.06d3
					mat(nelement,2)=2.527575d3
					mat(nelement,3)=2.512012d3	
				elseif (abs(xc(3))<=1.4d3) then
					mat(nelement,1)=4.688d3!3.06d3
					mat(nelement,2)=2.745681d3
					mat(nelement,3)=2.560331d3
				elseif (abs(xc(3))<=1.6d3) then
					mat(nelement,1)=5.032d3!3.06d3
					mat(nelement,2)=2.963788d3
					mat(nelement,3)=2.606060d3
				elseif (abs(xc(3))<=1.8d3) then
					mat(nelement,1)=5.376d3!3.06d3
					mat(nelement,2)=3.181894d3
					mat(nelement,3)=2.649501d3					
				elseif (abs(xc(3))<=4.73d3) then
					mat(nelement,1)=5.72d3!3.40d3
					mat(nelement,2)=3.40d3
					mat(nelement,3)=2.6d3	
				elseif (abs(xc(3))<=10.73d3) then
					mat(nelement,1)=6.18d3!3.62d3
					mat(nelement,2)=3.62d3
					mat(nelement,3)=2.8d3
				else
					mat(nelement,1)=6.32d3!3.67d3
					mat(nelement,2)=3.67d3!6.323
					mat(nelement,3)=2.8d3				
				endif
				mat(nelement,5)=mat(nelement,2)**2*mat(nelement,3)!miu=vs**2*rho
				mat(nelement,4)=mat(nelement,1)**2*mat(nelement,3)-2*mat(nelement,5)!lam=vp**2*rho-2*miu				
