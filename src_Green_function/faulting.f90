...
!For Cushing v2 Oct28.2019
		...
		boundxleft = (ipatchx - 1)*500.0d0  -4.25d3
		boundxright = ipatchx * 500.0d0 -4.25d3
		boundztop = -150.0d0 - (ipatchz - 1)*500.0d0
		boundzbottom = -150.0d0 - ipatchz*500.0d0
		if (x(1,isn)>boundxleft.and.x(1,isn)<boundxright.and.x(3,isn)>boundzbottom.and.x(3,isn)<boundztop) then
...
if((nt*dt<=5.0d0.and.mod(nt,4)==1).or.(nt*dt>5.and.nt*dt<10.and.mod(nt,100)==1)) then 
	write(mm,'(i6)') me
	mm = trim(adjustl(mm))
	foutmov='fslipout_'//mm
	open(9002+me,file=foutmov,form='formatted',status='unknown',position='append')
		write(9002+me,'(4e18.7e4)') ((fltslp(j,ifout),j=1,3),fltslp(7,ifout),ifout=1,nftnd)
endif
